...
export type Intent = 'direct_action' | 'coding_task' | 'settings' | 'question' | 'chitchat';
...
## 任务一：意图分类
判断用户意图，返回以下之一：
- direct_action: 简单文件操作，如复制、移动、保存到指定位置、重命名、删除文件。这些操作用 Node.js fs 模块就能完成，不需要编写代码或启动 CLI 工具。
- coding_task: 需要编写代码、运行命令、调试程序、处理图片（裁剪/缩放/格式转换等）、复杂文件操作等需要 CLI 工具的任务
- settings: 系统设置与管理操作，包括：查看/切换工作目录、切换 CLI 工具、切换模型/LLM 提供商、查看/设置人设、记忆管理（查看/搜索/清除记忆）、定时任务管理、查看当前状态、中止运行中的任务、查看帮助信息
